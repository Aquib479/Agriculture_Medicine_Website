...
const LoginForm = (props) => {
    ...
    return (
        <>
            <div className='bg-white/90 w-full h-[400px] md:w-[350px] m-2 md:m-0 md:h-[400px] md:absolute md:left-[12%] mt-[6rem] md:mt-[5rem] lg:mt-0 z-10'>
                <div className='col-span-1 px-4 py-2'>
                    ...
                    <div className="PhoneNumber flex flex-col my-2 relative mt-6">
                        <label htmlFor="Phone" className=' text-black font-semibold mx-2'>Email<span className='text-red-500'>*</span></label>
                        <input type="email" name='username' value={props.contact} placeholder='your email' className={`rounded-md py-2 px-3 bg-transparent border ${props.matched === '' ? 'border-slate-400' : 'border-red-600'}`} onChange={handleEmailInput} />
                    </div>
                    ...
                    <div className="formButton flex justify-center items-center mt-4">
                        <button type='submit' className={`w-full h-[2.5rem] text-white rounded-md text-[1rem] mt-4 font-bold transition-all ease-in duration-300 ${props.isValue === true ? 'bg-gray-400' : 'bg-blue-500 hover:bg-blue-600'}`} onClick={props.HandleLoginForm} disabled={props.isValue}>Submit</button>
                    </div>
                </div>
